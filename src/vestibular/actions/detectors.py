"""Action detection: ML classifier with rule-based fallback."""
...
_MODEL_PATH = Path(__file__).resolve().parent.parent.parent.parent / "data" / "action_classifier.pkl"
_CACHED_MODEL = None
...
def _detect_ml(kpts: List[KeypointsFrame], fps: float = 30.0
               ) -> Tuple[str, List[DetectionResult], Dict[str, float]] | None:
    """Try ML-based detection. Returns None if model unavailable."""
    model_data = _load_model()
    if model_data is None:
        return None

    feat = extract_features(kpts, fps=fps)
    if feat is None:
        return None
    ...
    for cls, prob in zip(classes, proba):
        candidates.append(DetectionResult(cls, float(prob), {}))
...
# ---------------------------------------------------------------------------
# Rule-based fallback (unchanged from before, but improved)
# ---------------------------------------------------------------------------
...
def _series(kpts: List[KeypointsFrame], conf_thr: float = 0.2) -> Dict[str, np.ndarray] | None:
    ...
    for f in kpts:
        ...
        hip.append(hip_xy); sh.append(sh_xy); nose.append(xy[NOSE])
        wr.append(wr_xy); ank.append(ank_xy)
...
def _detect_rules(kpts: List[KeypointsFrame]
                  ) -> Tuple[str, List[DetectionResult], Dict[str, float]]:
    ...
    feat = {
        "nose_dx": nose_dx, "hip_dx": hip_dx, "hip_dy": hip_dy,
        "hip_disp": hip_disp,
...
def detect_action_mvp(
    kpts: List[KeypointsFrame],
    fps: float = 30.0,
) -> Tuple[str, List[DetectionResult], Dict[str, float]]:
    """Detect the performed action. Uses ML model if available, else rules."""
    # Try ML first
    result = _detect_ml(kpts, fps)
    if result is not None:
        return result

    # Fallback to rules
    return _detect_rules(kpts)
